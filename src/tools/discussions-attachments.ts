...
import type {
  SmartsheetDiscussion,
  SmartsheetAttachment,
  SmartsheetPaginatedResponse,
} from "../types.js";

export function registerDiscussionAttachmentTools(server: McpServer): void {
...
Returns: Array of discussions with comments.`,
    ...
    async (params) => {
      try {
        ...
        if (params.include_comments) include.push("comments");

        const result = await smartsheetGet<
          SmartsheetPaginatedResponse<SmartsheetDiscussion>
        >(`/sheets/${params.sheet_id}/discussions`, {
    ...
    async (params) => {
      try {
        ...
        const result = await smartsheetGet<
          SmartsheetPaginatedResponse<SmartsheetDiscussion>
        >(
          `/sheets/${params.sheet_id}/rows/${params.row_id}/discussions`,
          include ? { include } : {}
        );
...
Returns: New comment ID and creation timestamp.`,
    ...
    async (params) => {
      try {
        const result = await smartsheetPost<{
          result: { id: number; text: string; createdAt: string };
          message: string;
        }>(
          `/sheets/${params.sheet_id}/discussions/${params.discussion_id}/comments`,
...
Returns: Deletion confirmation.`,
    ...
    async (params) => {
      try {
        const result = await smartsheetDelete<{ message: string; resultCode: number }>(
          `/sheets/${params.sheet_id}/comments/${params.comment_id}`
        );
...
Returns: Array of attachment metadata with download URLs (URLs expire after a short period).`,
    ...
    async (params) => {
      try {
        const result = await smartsheetGet<
          SmartsheetPaginatedResponse<SmartsheetAttachment>
        >(`/sheets/${params.sheet_id}/attachments`, {
          pageSize: params.page_size,
          page: params.page,
        });
...
Returns: Array of attachment metadata for the row.`,
    ...
    async (params) => {
      try {
        const result = await smartsheetGet<
          SmartsheetPaginatedResponse<SmartsheetAttachment>
        >(`/sheets/${params.sheet_id}/rows/${params.row_id}/attachments`);
...
Returns: Deletion confirmation.`,
    ...
    async (params) => {
      try {
        const result = await smartsheetDelete<{ message: string }>(
          `/sheets/${params.sheet_id}/attachments/${params.attachment_id}`
        );
