...
Returns: Chronological list of cell value changes with timestamps and user info.`,
    ...
    async (params) => {
      try {
        const result = await smartsheetGet<{
          data: CellHistoryEntry[];
          pageNumber: number;
          totalCount: number;
          totalPages: number;
        }>(
          `/sheets/${params.sheet_id}/rows/${params.row_id}/columns/${params.column_id}/history`,
          { pageSize: params.page_size, page: params.page }
        );
...
Returns: Current version number and last modification timestamp.`,
    ...
    async (params) => {
      try {
        const result = await smartsheetGet<{ version: number }>(
          `/sheets/${params.sheet_id}/version`
        );
