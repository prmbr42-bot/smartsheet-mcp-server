import { z } from "zod";
import { smartsheetGet, smartsheetPost, smartsheetPut, smartsheetDelete, handleApiError, truncateIfNeeded, } from "../services/smartsheet.js";
import { CHARACTER_LIMIT } from "../constants.js";
export function registerSheetTools(server) {
    // ── get_sheet_summary ────────────────────────────────────────────────────
    server.registerTool("smartsheet_get_sheet", {
...
Returns: Sheet metadata, columns array, and rows array with cell values.`,
        ...
        try {
            ...
            const sheet = await smartsheetGet(`/sheets/${params.sheet_id}`, queryParams);
            const output = {
                id: sheet.id,
                name: sheet.name,
    ...
    // ── list_sheets ──────────────────────────────────────────────────────────
    server.registerTool("smartsheet_list_sheets", {
...
Returns: Array of sheet summaries with IDs and metadata.`,
        ...
        try {
            ...
            const result = await smartsheetGet("/sheets", queryParams);
            const output = {
    ...
    // ── add_rows ─────────────────────────────────────────────────────────────
    server.registerTool("smartsheet_add_rows", {
...
Returns: Newly created row IDs and updated sheet version.`,
        ...
        try {
            ...
            const result = await smartsheetPost(`/sheets/${params.sheet_id}/rows`, payload);
    ...
    // ── update_rows ───────────────────────────────────────────────────────────
    server.registerTool("smartsheet_update_rows", {
...
Returns: Updated row data and new sheet version.`,
        ...
        try {
            ...
            const result = await smartsheetPut(`/sheets/${params.sheet_id}/rows`, payload);
    ...
    // ── delete_rows ───────────────────────────────────────────────────────────
    server.registerTool("smartsheet_delete_rows", {
...
Returns: Confirmation message and updated sheet version.`,
        ...
        try {
            ...
            const result = await smartsheetDelete(`/sheets/${params.sheet_id}/rows?rowIds=${params.row_ids.join(",")}&ignoreRowsNotFound=${params.ignore_rows_not_found}`);
            const output = {
                message: result.message,
                version: result.version,
    ...
    // ── get_columns ───────────────────────────────────────────────────────────
    server.registerTool("smartsheet_get_columns", {
...
Returns: Array of column definitions with IDs, types, and options.`,
        ...
        try {
            const result = await smartsheetGet(`/sheets/${params.sheet_id}/columns`, { includeAll: true });
            const output = {
                totalCount: result.totalCount,
    ...
    // ── add_columns ───────────────────────────────────────────────────────────
    server.registerTool("smartsheet_add_columns", {
...
Returns: Created column definitions with assigned IDs.`,
        ...
        try {
            ...
            const result = await smartsheetPost(`/sheets/${params.sheet_id}/columns`, payload);
            const output = {
