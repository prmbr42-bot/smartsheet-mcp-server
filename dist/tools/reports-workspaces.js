...
export function registerReportWorkspaceTools(server) {
    // ── get_report ────────────────────────────────────────────────────────────
    server.registerTool("smartsheet_get_report", {
...
Returns: Report metadata, columns, and rows with source sheet context.`,
        ...
        try {
            ...
            const report = await smartsheetGet(`/reports/${params.report_id}`, queryParams);
            const output = {
                id: report.id,
                name: report.name,
    ...
    // ── list_reports ──────────────────────────────────────────────────────────
    server.registerTool("smartsheet_list_reports", {
...
Returns: Array of report summaries with IDs.`,
        ...
        try {
            ...
            const result = await smartsheetGet("/reports", queryParams);
            const output = {
    ...
    // ── list_workspaces ───────────────────────────────────────────────────────
    server.registerTool("smartsheet_list_workspaces", {
...
Returns: Array of workspace summaries with IDs.`,
        ...
        try {
            const result = await smartsheetGet("/workspaces", {
                pageSize: params.page_size,
                page: params.page,
            });
            const output = {
    ...
    // ── browse_workspace ──────────────────────────────────────────────────────
    server.registerTool("smartsheet_browse_workspace", {
...
Returns: Workspace contents tree with IDs for all sheets, reports, folders, dashboards.`,
        ...
        try {
            const queryParams = {};
            if (params.load_all)
                queryParams.loadAll = true;
            const workspace = await smartsheetGet(`/workspaces/${params.workspace_id}`, queryParams);
            const output = {
                id: workspace.id,
                name: workspace.name,
    ...
    // ── browse_folder ─────────────────────────────────────────────────────────
    server.registerTool("smartsheet_browse_folder", {
...
Returns: Folder contents with IDs for all child resources.`,
        ...
        try {
            ...
            if (params.load_all)
                queryParams.loadAll = true;
            const folder = await smartsheetGet(`/folders/${params.folder_id}`, queryParams);
            const output = {
                id: folder.id,
                name: folder.name,
    ...
    // ── list_dashboards ───────────────────────────────────────────────────────
    server.registerTool("smartsheet_list_dashboards", {
...
Returns: Array of dashboard summaries with IDs and metadata.`,
        ...
        try {
            const result = await smartsheetGet("/sights", {
                pageSize: params.page_size,
                page: params.page,
            });
            const output = {
